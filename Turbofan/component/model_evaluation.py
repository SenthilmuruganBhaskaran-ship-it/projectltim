...
from Turbofan.exception import TurboException
from Turbofan.entity.config_entity import ModelEvaluationConfig
from Turbofan.entity.artifact_entity import DataIngestionArtifact, DataTransformationArtifact, ModelTrainerArtifact, ModelEvaluationArtifact
from Turbofan.util.util import write_yaml_file, read_yaml_file, load_object, load_numpy_array_data
from Turbofan.entity.model_factory import evaluate_regression_model
import os
import sys
import pandas as pd
...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifact:
        try:
            ...
            # Get the single best trained model from the trainer artifact
            trained_model_file_path = self.model_trainer_artifact.trained_model_file_path
            trained_model_object = load_object(file_path=trained_model_file_path)

            model_list = [trained_model_object]
            
            # Use the model name from the trainer artifact for the report key
            model_name = self.model_trainer_artifact.trained_model_name

            logging.info("Evaluating trained model on the test dataset.")
            evaluation_report = evaluate_regression_model(
                model_list=model_list,
                X_test=x_test,
                y_test=y_test
            )
            
            # Since the evaluate function might return generic keys, we rename it
            if evaluation_report:
                generic_model_key = list(evaluation_report.keys())[0]
                evaluation_report[model_name] = evaluation_report.pop(generic_model_key)
            
            logging.info(f"Model evaluation report: {evaluation_report}")

            # Prepare the final report content to be saved
            final_report = {
                "best_model_details": {
                    "model_path": trained_model_file_path,
                    "model_info": evaluation_report[model_name] if evaluation_report else "Evaluation failed."
                },
                "all_model_scores": evaluation_report 
            }
            
            # Save the full report
            model_evaluation_file_path = self.model_evaluation_config.model_evaluation_file_path
            write_yaml_file(file_path=model_evaluation_file_path, data=final_report)

            model_evaluation_artifact = ModelEvaluationArtifact(
                is_model_accepted=True, # Assuming acceptance if pipeline reaches this stage
                evaluated_model_path=trained_model_file_path
            )
            return model_evaluation_artifact
